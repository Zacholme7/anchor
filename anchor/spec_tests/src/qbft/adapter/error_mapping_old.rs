...
impl ErrorMapper {
    ...
    /// Map validation error to exact Go error string for QBFT message tests
    ...
    /// Enhanced error mapping for adapter errors with precise Go formatting
    pub fn map_adapter_error_to_go(&self, error: &super::AdapterError) -> String {
        match error {
            super::AdapterError::Qbft(qbft_error) => self.map_qbft_error_to_go(qbft_error),
            super::AdapterError::Validation(validation_error) => {
                // Try to parse validation error and map to Go format
                if validation_error.contains("identifier") {
                    "message identifier is invalid".to_string()
                } else if validation_error.contains("message type") {
                    "message type is invalid".to_string()
                } else if validation_error.contains("no signers") {
    ...
    /// Map string error messages to Go error format with enhanced pattern matching
    pub fn map_validation_error_strings(&self, error_strings: &[String]) -> Vec<String> {
        error_strings
            ...
            .map(|error_str| {
                ...
                } else if error_str.contains("ZeroRound") || error_str.starts_with("zero round") {
    ...
    fn get_context_specific_mappings() -> HashMap<(TestType, String), String> {
        ...
        // QBFT Message validation specific mappings
        mappings.insert(
            (TestType::QbftMessage, "MismatchedIdentifier".to_string()),
            "message identifier is invalid".to_string(),
        );
        mappings.insert(
            (TestType::QbftMessage, "UnknownQBFTMessageType".to_string()),
            "message type is invalid".to_string(),
        );
        mappings.insert(
            (TestType::QbftMessage, "NoSigners".to_string()),
            "no signers".to_string(),
        );
        mappings.insert(
            (TestType::QbftMessage, "DuplicatedSigner".to_string()),
            "duplicated signer".to_string(),
        );
        mappings.insert(
            (TestType::QbftMessage, "SignerNotInCommittee".to_string()),
            "signer not in committee".to_string(),
        );
        mappings.insert(
            (TestType::QbftMessage, "ZeroSigner".to_string()),
            "zero signer".to_string(),
        );
        mappings.insert(
            (TestType::QbftMessage, "ZeroRound".to_string()),
            "zero round".to_string(),
        );
        mappings.insert(
            (TestType::QbftMessage, "EmptyData".to_string()),
            "empty data".to_string(),
        );
        mappings.insert(
            (TestType::QbftMessage, "UndecodableMessageData".to_string()),
            "undecodable message data".to_string(),
        );
        mappings.insert(
            (TestType::QbftMessage, "SignatureVerification".to_string()),
            "signature verification failed".to_string(),
        );
        mappings.insert(
            (TestType::QbftMessage, "NilSSVMessage".to_string()),
            "nil SSVMessage".to_string(),
        );
