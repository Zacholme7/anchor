...
fn deserialize_optional_operator_ids<'de, D>(
    deserializer: D,
) -> Result<Option<Vec<OperatorId>>, D::Error>
where
    D: serde::Deserializer<'de>,
{
...
impl QbftMessageTest {
    ...
    fn create_signed_message(&self, test_msg: &TestMessage) -> Result<SignedSSVMessage, String> {
        ...
        if test_msg.ssv_message.msg_id.len() != 56 {
            return Err(format!(
                "Message ID must be 56 bytes, got {}",
                test_msg.ssv_message.msg_id.len()
            ));
        }
    ...
    fn run(&self) -> bool {
        ...
        for (i, test_msg) in self.messages.iter().enumerate() {
            ...
            if should_pass && !validation_passed {
                eprintln!(
                    "Test {} failed: Expected pass but got error: {}",
                    self.name, error_msg
                );
                return false;
            }
            ...
            if !should_pass && validation_passed {
                eprintln!(
                    "Test {} failed: Expected error '{}' but validation passed",
            ...
            if !should_pass {
                ...
                let error_matches = if expected_lower.contains("no signers") {
                    error_lower.contains("no signers")
                        || error_lower.contains("must have at least one signer")
                } else if expected_lower.contains("signer id 0 not allowed") {
                    // Accept either "signer ID 0" error or "signers not sorted" error when 0 is
                    // present
                    (error_lower.contains("signer")
                        && (error_lower.contains("0") || error_lower.contains("zero")))
                ...
                if !error_matches {
                    eprintln!(
                        ...
                        self.name, self.expected_error, error_msg
                    );
                    return false;
                }
            }
            ...
            if let Some(ref encoded_messages) = self.encoded_messages {
                ...
                {
                    eprintln!(
                        "Test {} failed: Encoding mismatch for message {}",
                        self.name, i
                    );
                    return false;
                }
            ...
            if let Some(ref expected_roots) = self.expected_roots {
                if i < expected_roots.len() && !self.test_root(test_msg, &expected_roots[i]) {
                    eprintln!(
                        "Test {} failed: Root computation mismatch for message {}",
                        self.name, i
                    );
                    return false;
                }
